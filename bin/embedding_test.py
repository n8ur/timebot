#!/usr/bin/env python3
# /usr/local/lib/timebot/bin/embedding_test.py
...
def parse_arguments():
    ...
    parser.add_argument('--url', default='https://febo.com/pages/hf_stability', help='URL to search for (web test)')
    parser.add_argument('--domain', default='febo.com', help='Domain to search for (web test)')
    parser.add_argument('--top-k', type=int, default=5, help='Number of results to return (web test)')
    parser.add_argument('--fuzzy', action='store_true', help='Enable fuzzy matching (web test)')
    parser.add_argument('--threshold', type=float, default=0.8, help='Metadata matching threshold (web test)')
    parser.add_argument('--mode', default='all', choices=['all', 'web'], help='Test mode: all (full suite) or web (web metadata tests only)')
...
def print_result(result: Dict[str, Any], detailed: bool = False):
    ...
    if detailed:
        ...
        for key, value in metadata.items():
            if key not in ["doc_type", "from", "subject", "date", "title", "author", "publisher", "chunk_number", "total_chunks", "domain", "source_url", "captured_at"]:
                print(f"  {key}: {value}")
    print("-" * 40)
...
def test_api_info(api_url: str):
    ...
    try:
        endpoints = [f"{api_url}/info", f"{api_url}/api/info", f"{api_url}/v1/info", f"{api_url}/api/v1/info"]
        response = None
...
def test_metadata_search(api_url: str, metadata: Dict[str, Any], content_query: str = "", 
                        collection_filter: str = "all", top_k: int = 5, fuzzy: bool = True, threshold: float = 0.8):
    ...
    try:
        ...
        if result.get('results'):
            ...
            for i, item in enumerate(result.get('results')[:5]):
                print(f"\nResult #{i+1}:")
                print_result(item, detailed=True)
        else:
            print("\nNo results found.")
...
def test_web_metadata_suite(api_url: str, args):
    # Test URL search
...
def main():
    ...
    if args.mode == 'web':
        test_web_metadata_suite(base_url, args)
    else:
